...
async def handle_start(client, message):
    ...
    try:
        ...
        if not existing_user:
            ...
            logger.info(f"User {user_id} already exists in database: {existing_user}")
        ...
        await message.reply("An error occurred while starting the bot. Please try again later.")
...
async def check_sub_callback(client, callback_query):
    ...
    try:
        ...
        logger.error(f"Error in check_sub_callback for user {user_id}: {e}")
        await callback_query.message.reply("An error occurred while checking subscription. Please try again.")
